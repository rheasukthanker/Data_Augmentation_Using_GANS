...
def load_real_samples():
    ...
    del all_images
    del all_labels
    train_images, _, y, _ = train_test_split(X,
                                             y,
                                             test_size=0.25,
                                             random_state=42)
...
def define_generator(latent_dim, n_classes=2):
    ...
    gen = Conv2DTranspose(192, (3, 3), strides=(2, 2),
                          kernel_initializer=init)(merge)
    gen = BatchNormalization()(gen)
    gen = Activation('relu')(gen)
    gen = Conv2DTranspose(192, (4, 4), strides=(2, 2),
...
def train(g_model,
    ...
    for i in range(n_epochs):
        ...
        if ((i + 1) % 2) == 0:
            g_model.save('acgan_generator_sampling_25_75_med_epoch' +
                         str(i + 1) + '.h5')  #save model every second epoch
