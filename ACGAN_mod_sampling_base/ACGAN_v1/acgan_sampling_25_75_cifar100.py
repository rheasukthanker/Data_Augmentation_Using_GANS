# Modified Auxiliary Classifier gan (ac-gan) on cifar100 with 25% real data and 75% fake data
import numpy as np
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=300, n_batch=64):
	...
	for i in range(n_epochs):
		for j in range(bat_per_epo):
			...
			[X_real, labels_real], y_real = generate_real_samples(dataset, quarter_batch)
			_,d_r1,d_r2 = d_model.train_on_batch(X_real, [y_real, labels_real])
            
			[X_fake, labels_fake], y_fake = generate_fake_samples(g_model, latent_dim, 3*quarter_batch)
		...
		if ((i+1)%2)==0:
			g_model.save('acgan_generator_sampling_25_75_cifar100_epoch'+str(i+1)+'.h5')
