...
def generate_latent_representations(num_samples,real_samples,real_labels,perc_real,n_classes):
    #get latent representations which is a mixture of real z and random/fake z
...
def generate_imbalance(trainX, trainy, class_cur, perc):
    #artificially sparsify the data
...
def generate_latent_points(latent_dim, n_samples, n_classes=2):
    # generate points in the latent space (fully random/fake)
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=300, n_batch=64):
    ...
    num_samples=n_batch
    #load latent vectors obtained from pretrained ALI encoder
    ...
    for i in range(n_epochs):
       for j in range(bat_per_epo):
        ...
        [z_input, z_labels] = generate_latent_representations(num_samples,real_samples,real_labels,0.75,2)
    ...
    with open("generator_loss_med_test1.pkl","wb") as f:
         pickle.dump(lossg, f)
    with open("discriminator_real_loss_med_test1.pkl","wb") as f:
         pickle.dump(lossd_real,f)
    with open("discriminator_fake_loss_med_test1.pkl", "wb") as f:
...
#load pretrained model weights (architecture of generator same as ALI)
