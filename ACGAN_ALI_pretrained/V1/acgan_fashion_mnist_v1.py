...
def generate_imbalance(trainX, trainy, class_cur, perc):
        #generate imbalance given the percentage 
...
def generate_latent_representations(num_samples,real_samples,real_labels,perc_real,n_classes):
        #generate latent representations which are mixture of real and fake z
...
def define_discriminator(in_shape=(28, 28, 1), n_classes=10):
	...
	# compile model
	opt = optimizers.Adam(lr=1e-4, beta_1=0.5)
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=200, n_batch=64):
	...
	num_samples=n_batch
        #load real latent z obtained from the pre-trained ALI encoder
	...
	for i in range(n_epochs):
		for j in range(bat_per_epo):
		 ...
		 [z_input, z_labels] = generate_latent_representations(num_samples,real_samples,real_labels,0.75,10)
		...
		g_model.save('acgan_fashion_mnist_model_' + str(i) + '.h5')
	with open("generator_loss_fm_test1.pkl","wb") as f:
          pickle.dump(lossg, f)
	with open("discriminator_real_loss_fm_test1.pkl","wb") as f:
	      pickle.dump(lossd_real,f)
	with open("discriminator_fake_loss_fm_test1.pkl", "wb") as f:
...
#load pretrained weights from ALI generator
