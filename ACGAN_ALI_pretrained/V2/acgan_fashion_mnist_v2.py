...
import pickle
# Sparsify 1,3,7 (50%,65%,80%), use 7000 round int
import tensorflow as tf
def binary_crossentropy_masked(y_true, y_pred):
    #define loss function for masking out the ALI real-fake objective for real images 
...
def generate_imbalance(trainX, trainy, class_cur, perc):
        #generate imbalance artificially
...
def generate_latent_representations(num_samples,real_samples,real_labels,perc_real,n_classes):
        #get latent representations as a mixture of real and fake z
	...
	real_labels=np.expand_dims(real_labels,1)
...
def define_discriminator(in_shape=(28, 28, 1), n_classes=10):
	...
	c = ConvMaxout(n_piece=2)(c)
	c = Dropout(0.5)(c)
	out1 = Conv2D(1, (1, 1), strides=(1, 1), activation='sigmoid')(c)
	c2 = Dropout(0.5)(x)
	...
	c2 = ConvMaxout(n_piece=2)(c2)
	c2 = Dropout(0.5)(c2)
	out2 = Conv2D(n_classes, (1, 1), strides=(1, 1), activation='softmax')(c2)
	out1 = Reshape((1,))(out1)
	out2 = Reshape((n_classes,))(out2)
...
def define_generator():
	...
	label = Input(shape=(1,))
	class_embs = Embedding(10, 64, input_length=1)(label)
...
def load_real_samples(classes, percs):
	...
	for i in range(len(classes)):
		trainX, trainy = generate_imbalance(trainX, trainy, classes[i], percs[i])
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=300, n_batch=64):
	...
	num_samples=n_batch
        #Load real z obtained for training images using the pretrined ALI Encoder
	with open('../../data/z_train_fashion_mnist.pkl', 'rb') as f:
		real_samples=pickle.load(f)
	with open('../../data/z_train_labels_fashion_mnist.pkl', 'rb') as f:
	...
	for i in range(len(classes_to_sparsify)):
		real_samples, real_labels = generate_imbalance(real_samples, real_labels, classes_to_sparsify[i], sparsify_percentage[i])
	for i in range(n_epochs):
		for j in range(bat_per_epo):
		 # get randomly selected 'real' samples
		 [X_real, labels_real], y_real = generate_real_samples(dataset, int(half_batch/2))
		 # update discriminator model weights
		 _, d_r1, d_r2 = d_model.train_on_batch(X_real, [y_real, labels_real])
		 # generate 'fake' examples
		 [X_fake, labels_fake], y_fake = generate_fake_samples(g_model, latent_dim, int(half_batch/2))
		 # update discriminator model weights
		 _, d_f, d_f2 = d_model.train_on_batch(X_fake, [y_fake, labels_fake])
		 # prepare points in latent space as input for the generator
		 [z_input, z_labels] = generate_latent_representations(num_samples,real_samples,real_labels,0.25,10)
		...
		g_model.save('acgan_fashion_mnist_model' + str(i) + '.h5')
	with open("generator_loss_fm_test3.pkl", "wb") as f:
		pickle.dump(lossg, f)
	with open("discriminator_real_loss_fm_test3.pkl", "wb") as f:
		pickle.dump(lossd_real, f)
	with open("discriminator_fake_loss_fm_test3.pkl", "wb") as f:
...
#Load pre-trained ALI generators weights
generator.load_weights('../../data/Fashion_Mnist_ALI_weights/xgen_weights.h5')
# create the gan
