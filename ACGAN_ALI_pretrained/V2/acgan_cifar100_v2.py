...
def binary_crossentropy_masked(y_true, y_pred):
    #define objective to mask the real fake loss for real z
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=200, n_batch=64):
    ...
    #load real z obtained as output from ALI pretrained Encoder
    ...
    for i in range(n_epochs):
      for j in range(bat_per_epo):
        ...
        [z_input, z_labels] = generate_latent_representations(num_samples,real_samples,real_labels,0.25,100)
    ...
    with open("generator_loss_cifar_test3.pkl","wb") as f:
         pickle.dump(lossg, f)
    with open("discriminator_real_loss_cifar_test3.pkl","wb") as f:
         pickle.dump(lossd_real,f)
    with open("discriminator_fake_loss_cifar_test3.pkl", "wb") as f:
         pickle.dump(lossd_fake, f)

# size of the latent space
latent_dim = 64
classes_to_sparsify = [83, 69, 35, 34, 2, 90, 37, 49, 23, 26, 41, 62, 33, 12, 84, 50, 57, 74, 93, 40]
sparsify_percentage = [0.65, 0.65, 0.65, 0.65, 0.65, 0.7, 0.7, 0.7, 0.7, 0.7, 0.75, 0.75, 0.75, 0.75, 0.75, 0.8, 0.8,
                       0.8, 0.8, 0.8]
...
#load weights from pretrained ALI generator (architecture kept same)
