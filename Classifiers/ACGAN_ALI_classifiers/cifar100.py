...
parser.add_argument('--model_name', nargs="?", type=str, default='../../data/ACGAN_ALI_pretrained/V1/acgan_ali_v1_generator_cifar100.h5', help='model_name')
...
mcp_save = tf.keras.callbacks.ModelCheckpoint('best.mdl_wts_cifar1002.hdf5', save_best_only=True, monitor='val_loss', mode='min')
...
gan_labels=[]
for i in range(len(classes_to_sparsify)):
    [X,labels]=get_images_from_generator(classes_to_sparsify[i],model_name,int((1-sparsify_percentage[i])*500),latent_dim)
...
model.load_weights("best.mdl_wts_cifar1002.hdf5")
