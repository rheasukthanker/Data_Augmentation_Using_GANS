...
def top2_acc(labels, logits):
    return tf.keras.metrics.sparse_top_k_categorical_accuracy(y_true=labels,
                                                              y_pred=logits,
                                                              k=2)
...
mcp_save = tf.keras.callbacks.ModelCheckpoint(
    'best.mdl_wts_fashion_mnist.hdf5',
    save_best_only=True,
    monitor='val_loss',
    mode='min')
...
(train_images,
 ...
 )  #Change this line to modify train and test input
#Normalize data
...
for i in range(len(classes_to_sparsify)):
    [X, labels
     ] = get_images_from_generator(classes_to_sparsify[i], model_name,
                                   int((1 - sparsify_percentage[i]) * 6000),
                                   latent_dim)
