...
#Fit model
model.fit(np.expand_dims(train_images,3), train_labels,callbacks=[early_stopping, mcp_save],validation_split=0.1,batch_size=batch_size, epochs=epochs)
#Get test accuracy
model.load_weights("best.mdl_wts.hdf5")
test_loss, test_acc = model.evaluate(np.expand_dims(test_images,3),test_labels,verbose=2)
