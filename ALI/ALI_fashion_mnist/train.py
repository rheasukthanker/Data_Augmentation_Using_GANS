...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(0, num_train, args.batch_size):
            ...
            # train discriminater
            d_loss = gan_d.train_on_batch([x,l,z], np.zeros((len(x), 1, 1, 2)))
            # train generaters
            g_loss = gan_g.train_on_batch([x,l,z], np.zeros((len(x), 1, 1, 2)))
        ...
        if (epoch+1) % args.snap_freq == 0:
            ...
            img = utils.reconstruct_img(np.expand_dims(x_test,3),y_test, xgen, zgen)
